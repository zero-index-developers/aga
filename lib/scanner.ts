...
const TYPE_CONFIG: Record<string, { color: string; parent: string }> = {
...
export async function scanProject(rootPath: string) {
  const nodes: Node[] = [
    // Folder Groups
    { id: 'group-pages', data: { label: '/app (Pages)', type: 'folder', path: 'app/', color: '' }, position: { x: 50, y: 50 }, style: { width: 800, height: 120 }, className: 'bg-purple-500/5 border-purple-500/20 rounded-xl z-[-1]', type: 'group' },
    { id: 'group-api', data: { label: '/api (Routes)', type: 'folder', path: 'app/api/', color: '' }, position: { x: 50, y: 190 }, style: { width: 800, height: 120 }, className: 'bg-emerald-500/5 border-emerald-500/20 rounded-xl z-[-1]', type: 'group' },
    { id: 'group-components', data: { label: '/components', type: 'folder', path: 'components/', color: '' }, position: { x: 50, y: 330 }, style: { width: 800, height: 120 }, className: 'bg-blue-500/5 border-blue-500/20 rounded-xl z-[-1]', type: 'group' },
    { id: 'group-ui', data: { label: '/components/ui', type: 'folder', path: 'components/ui/', color: '' }, position: { x: 50, y: 470 }, style: { width: 800, height: 120 }, className: 'bg-slate-500/5 border-slate-500/20 rounded-xl z-[-1]', type: 'group' },
    { id: 'group-lib', data: { label: '/lib', type: 'folder', path: 'lib/', color: '' }, position: { x: 50, y: 610 }, style: { width: 800, height: 120 }, className: 'bg-amber-500/5 border-amber-500/20 rounded-xl z-[-1]', type: 'group' },
  ];
  const edges: Edge[] = [];

  const fileMap = new Map<string, string>(); // path -> id
  ...
  // Dynamic Grid Layout Calculation (Wrapped)
  const MAX_COLS = 5;
  const COL_WIDTH = 160;
  const ROW_HEIGHT = 70;
  ...
  // Update folder dimensions based on grid content
  nodes.forEach(node => {
